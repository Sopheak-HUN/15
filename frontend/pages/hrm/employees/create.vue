...
// Lookups for departments and positions
...
// Date conversion helper for DatePicker Date object -> YYYY-MM-DD string
...
const onSubmit = handleSubmit(
  async (values) => {
    try {
      ...
      toast.add({ severity: 'success', summary: t('hrm.employees.toast.created'), life: 2500 })
  ...
  ({ errors }) => {
    // Toast the first error details clearly
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-surface-900 dark:text-surface-50">{{ t('hrm.employees.dialog.createTitle') }}</h1>
        <p class="text-surface-500 text-sm mt-0.5">Fill out information to onboard new employee</p>
      </div>
    </div>
      ...
      <template #content>
        <form class="space-y-6" @submit="onSubmit">
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <Button type="submit" :label="t('common.create')" icon="pi pi-check" :loading="isSubmitting" class="hover:scale-[1.02] active:scale-[0.98] transition-transform duration-100" />
          </div>
        </form>
      </template>
    </Card>
