...
import { z } from 'zod'
import type { Employee, Position } from '~/types/hrm'
import type { GeoUnit } from '~/composables/useCambodiaGeo'

// Mode-aware wizard. In `create` mode (default) the form is empty and
// submit POSTs to /api/hrm/employees. In `edit` mode the parent passes
// the loaded employee via `initial`; the form pre-populates and submit
// PUTs to /api/hrm/employees/{id}.
//
// Pre-population details:
//  - geo cascades (current / permanent / emergency) seed their dropdown
//    lists in onMounted by replaying listDistricts / listCommunes /
//    listVillages for whatever codes the employee has.
//  - photo: edit mode shows the existing `photo_url` and treats new
//    `photoFile` selections as a replacement. If the user doesn't touch
//    the input, no photo_temp_key is sent and the backend leaves it.
...
const schema = computed(() => {
  ...
  return toTypedSchema(
    z.object({
      ...
      // In edit mode the photo is optional — an existing `photo_url`
      // means the user already uploaded one. In create mode we still
      // require a File.
      photo:         props.mode === 'edit'
        ? z.any().optional()
        : z.any().refine((f) => f instanceof File, { message: req }),
...
// Map an `initial` employee (with relations from GET /employees/{id})
// onto the flat wizard form keys. In create mode every key falls back
// to an empty / default value.
//
// Backend `employment_type` enum (`full_time | part_time | contract |
// intern`) is NOT the same as the wizard's `contract_type` (`work |
// fdc | udc | probation | internship | consulting`). The active
// contract row carries the true `contract_type`, so we read from
// there; the employment_type is only used as a fallback during
// submit via employmentTypeFromContract().
const initial = props.initial
const initEducation = initial?.educations?.[0] ?? null
const initContract  = initial?.active_contract ?? null
const buildInitial = () => ({
  first_name:    initial?.first_name ?? '',
  last_name:     initial?.last_name ?? '',
  first_name_kh: initial?.first_name_kh ?? '',
  last_name_kh:  initial?.last_name_kh ?? '',
  gender:        initial?.gender ?? '',
  date_of_birth: (initial?.date_of_birth ?? null) as unknown as string,
  joined_date:   initial?.hire_date ?? today,
  nssf_id:       initial?.nssf_id ?? '',
  department_id: (initial?.department_id ?? undefined) as string | undefined,
  position_id:   (initial?.position_id ?? undefined) as string | undefined,
  ...
  salary:        (initial?.base_salary as number | null | undefined) ?? null,
  // In edit mode the photo input is OPTIONAL — the schema is loosened
  // below to allow `undefined` when an existing photo_url is present.
  photo:         undefined as unknown as File,
  home_number:   initial?.current_address?.home_number ?? '',
  street:        initial?.current_address?.street ?? '',
  province_id:   initial?.current_address?.province_code ?? '',
  district_id:   initial?.current_address?.district_code ?? '',
  commune_id:    initial?.current_address?.commune_code ?? '',
  village_id:    initial?.current_address?.village_code ?? '',
  lat:           (initial?.current_address?.lat as number | null | undefined) ?? null,
  lng:           (initial?.current_address?.lng as number | null | undefined) ?? null,
  perm_home_number: initial?.permanent_address?.home_number ?? '',
  perm_street:      initial?.permanent_address?.street ?? '',
  perm_province_id: initial?.permanent_address?.province_code ?? '',
  perm_district_id: initial?.permanent_address?.district_code ?? '',
  perm_commune_id:  initial?.permanent_address?.commune_code ?? '',
  perm_village_id:  initial?.permanent_address?.village_code ?? '',
  perm_lat:         (initial?.permanent_address?.lat as number | null | undefined) ?? null,
  perm_lng:         (initial?.permanent_address?.lng as number | null | undefined) ?? null,
  identification_type: initial?.identification_type ?? '',
  id_card_number:      initial?.id_card_number ?? '',
  id_issued_date:      initial?.id_issued_date ?? null,
  id_issued_by:        initial?.id_issued_by ?? '',
  id_issued_place:     initial?.id_issued_place ?? '',
  religion:        initial?.religion ?? '',
  marital_status:  initial?.marital_status ?? '',
  blood_group:     initial?.blood_group ?? '',
  spouse_name:          initial?.spouse?.name ?? '',
  spouse_date_of_birth: initial?.spouse?.date_of_birth ?? null,
  spouse_education:     initial?.spouse?.education ?? '',
  spouse_occupation:    initial?.spouse?.occupation ?? '',
  children_count:  initial?.children_count ?? 0,
  contact_phone:   initial?.contact_phone ?? '',
  er_father_name:       initial?.emergency_contact?.father_name ?? '',
  er_mother_name:       initial?.emergency_contact?.mother_name ?? '',
  er_father_occupation: initial?.emergency_contact?.father_occupation ?? '',
  er_mother_occupation: initial?.emergency_contact?.mother_occupation ?? '',
  er_home:        initial?.emergency_address?.home_number ?? '',
  er_street:      initial?.emergency_address?.street ?? '',
  er_province_id: initial?.emergency_address?.province_code ?? '',
  er_district_id: initial?.emergency_address?.district_code ?? '',
  er_commune_id:  initial?.emergency_address?.commune_code ?? '',
  er_village_id:  initial?.emergency_address?.village_code ?? '',
  er_group:        initial?.emergency_address?.group ?? '',
  er_phone_number: initial?.emergency_contact?.phone_number ?? '',
  er_home_phone:   initial?.emergency_contact?.home_phone ?? '',
  education_level:   initEducation?.level ?? '',
  major_subject:     initEducation?.major_subject ?? '',
  education_status:  initEducation?.status ?? '',
  university_school: initEducation?.university_school ?? '',
  contract_type:    initContract?.type ?? '',
  contract_start:   initContract?.start_date ?? today,
  contract_end:     initContract?.end_date ?? null,
  contract_comment: initContract?.comment ?? '',
})
...
// In edit mode we seed the preview from the presigned `photo_url`
// returned by the API. It stays visible until the user picks a new
// file (which replaces the preview) or hits Clear (which only clears
// the local preview — the existing photo on the server is left alone
// unless the user submits without picking a new one and without
// touching the photo input, in which case no photo_temp_key is sent
// and the backend keeps the old photo).
const photoPreview = ref<string | null>(props.initial?.photo_url ?? null)
...
onMounted(async () => {
  ...
  // In edit mode, pre-fetch every cascade list the existing employee
  // already has codes for, so the dropdowns show the human-readable
  // names instead of an empty list with an opaque code selected. The
  // composable caches per parent code so repeated fetches are free.
  if (isEdit.value) {
    const seed = async (
      provinceCode: string | null | undefined,
      districtCode: string | null | undefined,
      communeCode: string | null | undefined,
      lists: { d: typeof districts, c: typeof communes, v: typeof villages },
    ) => {
      if (provinceCode) lists.d.value = await geo.listDistricts(provinceCode)
      if (districtCode) lists.c.value = await geo.listCommunes(districtCode)
      if (communeCode)  lists.v.value = await geo.listVillages(communeCode)
    }
    await Promise.all([
      seed(
        initial?.current_address?.province_code,
        initial?.current_address?.district_code,
        initial?.current_address?.commune_code,
        { d: districts, c: communes, v: villages },
      ),
      seed(
        initial?.permanent_address?.province_code,
        initial?.permanent_address?.district_code,
        initial?.permanent_address?.commune_code,
        { d: permDistricts, c: permCommunes, v: permVillages },
      ),
      seed(
        initial?.emergency_address?.province_code,
        initial?.emergency_address?.district_code,
        initial?.emergency_address?.commune_code,
        { d: erDistricts, c: erCommunes, v: erVillages },
      ),
    ])
  }
})
...
const onSubmit = handleSubmit(
  async (v) => {
    ...
    try {
      const resp = isEdit.value && initial
        ? await hrm.updateEmployee(initial.id, payload)
        : await hrm.createEmployee(payload)
      toast.add({
        severity: 'success',
        summary: isEdit.value
          ? t('hrm.employees.toast.updated')
          : t('hrm.employees.toast.created'),
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold tracking-tight">
          {{ isEdit ? t('hrm.employees.wizard.editTitle') : t('hrm.employees.wizard.title') }}
        </h1>
        <p class="text-surface-500 text-sm mt-0.5">
          {{ isEdit ? t('hrm.employees.wizard.editSubtitle') : t('hrm.employees.wizard.subtitle') }}
        </p>
      </div>
    </div>
            ...
            <StepPanel v-slot="{ activateCallback: _ }" value="7">
              <form class="space-y-6 pt-4" @submit.prevent="onSubmit">
                ...
                <div class="flex justify-between items-center pt-4 border-t border-surface-200 dark:border-surface-800">
                  ...
                  <Button
                    type="submit"
                    :label="isEdit ? t('hrm.employees.wizard.actions.update') : t('hrm.employees.wizard.actions.submit')"
                    icon="pi pi-check"
